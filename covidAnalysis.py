...
sys.stdout = open(sys.stdout.fileno(), mode='w', encoding='utf8', buffering=1) # this thing will help to generate bangla or other native language on terminal
...
data_list = ['008','009','010','011','012', '101', '102']
data_list = ['008','009','010','011','012','101', '102','103','104','105','106']
...
lifestage_df = spark.read.csv('app/lifestage/all/all/all/app_lifestage.csv', header=True)
...
# brq = spark.read.parquet('s3a://ada-prod-data/etl/data/brq/agg/agg_brq/daily/BD/202106' + date + '/*.parquet')
for date in data_list:
    print('+=============' + date + '===============')
    brq = spark.read.parquet('brq/agg/agg_brq/monthly/BD/202' + date + '/*.parquet')
    df=brq.select('ifa').cache()
    connection = brq.select('ifa', F.explode('connection.req_carrier_name').alias('telco'))
    ...
    df_age = spark.read.parquet(
        'brq/sub/demographics/monthly/' + country + '/' + year + '/age/*.parquet')
    ...
    df_gender = spark.read.parquet(
        'brq/sub/demographics/monthly/' + country + '/' + year + '/gender/*.parquet')
    ...
    print('xxxxxxxxxxxxxx' + date + ' Done xxxxxxxxxxxxxxx')
#TOP APP
for date in data_list:
    print('+============= ' + date + ' ===============')
    brq = spark.read.parquet('/agg/agg_brq/monthly/BD/202' + date + '/*.parquet')
    df = brq.select('ifa', F.explode('app')).select('ifa', 'col.*')
    app = df.join(finalapp_df, on='bundle', how='left').cache()
    persona_app = app
    freq_beh = persona_app.groupBy('app_l1_name').agg(F.countDistinct('ifa').alias('freq')).sort('freq',
                                                                                                 ascending=False)
    freq_beh1 = freq_beh.filter(freq_beh['app_l1_name'] != 'null')
    ...
    print(date + ' of top app')
    topApp_df1 = app.filter((app.app_l2_name.isin(list1)) | (app.app_l1_name.isin(list1)))
    top_app1 = topApp_df1.groupBy('asn').agg(F.countDistinct('ifa').alias('ifa')).sort('ifa', ascending=False)
    ...
    top_app2 = topApp_df2.groupBy('asn').agg(F.countDistinct('ifa').alias('ifa')).sort('ifa', ascending=False)
    ...
    top_app3 = topApp_df3.groupBy('asn').agg(F.countDistinct('ifa').alias('ifa')).sort('ifa', ascending=False)
    ...
    top_app4 = topApp_df4.groupBy('asn').agg(F.countDistinct('ifa').alias('ifa')).sort('ifa', ascending=False)
    ...
    top_app5 = topApp_df5.groupBy('asn').agg(F.countDistinct('ifa').alias('ifa')).sort('ifa', ascending=False)
...
for date in data_list:
    print('+============= ' + date + ' ===============')
    df = spark.read.csv('s3a://ada-bd-emr/result/20210608/' + date + '/*.csv', header=True)
    length = len(date)
    last_three = date[length - 3:]
    brq = spark.read.parquet('agg/agg_brq/monthly/BD/202' + last_three + '/*.parquet')
    df1 = brq.select('ifa', F.explode('app')).select('ifa', 'col.*')
    app = df1.join(finalapp_df, on='bundle', how='left').cache()
    persona_app = df.join(app, on='ifa')
    TopGame = persona_app.filter(persona_app['app_l1_name'] == 'Games')
...
for i in data_list:
    df = spark.read.parquet('brq/agg/agg_brq/monthly/BD/202008/*.parquet')
    brq= df.select('ifa', F.explode('app.first_seen').alias('dev_datetime_local'))
    split_col = F.split(brq['dev_datetime_local'], ' ')
    df1 = brq.withColumn('dev_date', split_col.getItem(0))
    ...
    print('Date and time for {} done'.format(i))
...
for date in data_list:
    ...
    topDevice.show(20, False)
    print('---------- Os Ration ---------')
    topOs = deviceInfo.groupBy('major_os').agg(F.countDistinct('ifa').alias('ifa')).sort('ifa', ascending=False)
    topOs.show()
    print('xxxxxxxxxxxxxx' + date + ' Done xxxxxxxxxxxxxxx')
